(()=>{
  ...
  if(project!=='/'&&current.startsWith(project)) current=current.slice(project.length);
  else current=current.replace(/^\//,'');

  current=current.replace(/^\/+|\/+$/g,'');
  if(current.endsWith('/index.html')) current=current.slice(0,-11).replace(/\/$/,'');
  else if(current==='index.html') current='';
  ...
  if(nav){
    nav.querySelectorAll('[data-path]').forEach(a=>{
      ...
      if(target===current){
        ...
        if(trigger) trigger.classList.add('nav-section-current');
      }
    });
  }
